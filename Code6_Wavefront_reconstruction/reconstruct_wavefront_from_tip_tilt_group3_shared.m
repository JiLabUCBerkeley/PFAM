...
grpDir     = fullfile(dataDir, '3_Group');          % round1/2/3 .mat files
...
% Ground truth (system correction)
GT = load(fullfile(dataDir, 'GT_syscorr.mat'), 'IrisAO_PTT');
IrisAO_PTT_GT = GT.IrisAO_PTT;
...
round1_tip_tilt = load(fullfile(grpDir, 'round1.mat'), 'tip_tilt_saved').tip_tilt_saved;
round2_tip_tilt = load(fullfile(grpDir, 'round2.mat'), 'tip_tilt_saved').tip_tilt_saved;
round3_tip_tilt = load(fullfile(grpDir, 'round3.mat'), 'tip_tilt_saved').tip_tilt_saved;

% Center by grand mean (remove global bias)
round123_center = mean([round1_tip_tilt; round2_tip_tilt; round3_tip_tilt], 1, 'omitnan');
round1_tip_tilt = round1_tip_tilt - round123_center;
round2_tip_tilt = round2_tip_tilt - round123_center;
round3_tip_tilt = round3_tip_tilt - round123_center;
...
tip_tilt_total3 = NaN(Param.SegN, 2);
...
for ii = 1:numel(target_segment3)
    seg = target_segment3(ii);
    [neigh, ~] = Find6NeighbrIdx(seg, Param, Convert_hex2jk, Convert_jk2hex);
    if ~(seg == 1 || seg == 2), neigh(neigh==1) = 163; end
    bundle = [seg; neigh];
    round3_segment = [round3_segment; bundle']; 
    tip_tilt_total3(bundle, :) = repmat(round3_tip_tilt(ii,:), 7, 1);
end

used_segment_total = unique([round1_segment; round2_segment; round3_segment]);
...
for t = 1:numel(target_segment3)
    PTT_mode_round3(2, round3_segment(t,:), :) = row2_block;
    PTT_mode_round3(3, round3_segment(t,:), :) = row3_block;
end
...
round3_PTT = tip_tilt_total3(:,1).*squeeze(PTT_mode_round3(2,:,:)) + ...
             tip_tilt_total3(:,2).*squeeze(PTT_mode_round3(3,:,:));

PTT_total = zeros(3, Param.SegN, 3);
